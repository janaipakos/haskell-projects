...
groupings board = (rows board) ++ (cols board) ++ [leftDiagonal board, rightDiagonal board]

getWinner :: [Square] -> Maybe Player
getWinner squares 
                  | (length $ filter (== X) squares) == 3 = Just PlayerX
...
isDraw :: Board -> Bool
isDraw board 
             | getGameWinner board == Nothing = null $ filter (== Empty) $ concat board
             | otherwise = False
...
doMove board player (row, col) 
                               | row < 0 || row > 2 = Nothing
                               | col < 0 || col > 2 = Nothing
                               | board !! row !! col /= Empty = Nothing
                               | otherwise = Just (newBoard board player (row, col))
...
takeGameStep board player 
                          | getGameWinner board /= Nothing = endWithWin board player
                          ...
                          | otherwise = do
                            ...
                            if (coord == Nothing) then do
                                ...
                                let nextBoard = doMove board player $ fromJust coord
                                if (nextBoard == Nothing) then do
                                    putStrLn "Invalid move!"
                                    takeGameStep board player
                                else do
                                    takeGameStep (fromJust nextBoard) $ nextPlayer player
...
main = do
    ...
    takeGameStep emptyBoard PlayerX